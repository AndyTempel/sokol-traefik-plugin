...
<script nonce="{{SOKOL_CSP_NONCE}}">(()=>{const r=document.getElementById('challenge'),w=document.getElementById('widget'),s=document.getElementById('status'),n=document.getElementById('warning');let q=false;const blocked=()=>{n.hidden=false};const fail=()=>{blocked();s.textContent='Verification did not complete. Please retry.';q=false};const payload=x=>x&&x.payload||r.querySelector('input[name="sokol"]')?.value;const complete=async x=>{if(q)return;q=true;s.textContent='Finishing verification…';try{const p=payload(x);if(!p)throw Error('payload');const u=new URL(r.dataset.url,location.href),z=await fetch(u.pathname+'/verify',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({challenge_token:r.dataset.token,resource_id:r.dataset.resource,site_id:r.dataset.site,path:r.dataset.path,payload:p})}),j=await z.json();if(!z.ok||!j.verified)throw Error('rejected');const d=new URL(location.href);d.pathname=r.dataset.path;d.search='';d.hash='';location.assign(d.href)}catch(e){fail()}};w.addEventListener('verified',e=>complete(e.detail));w.addEventListener('statechange',e=>{if(e.detail&&e.detail.state==='verified')complete(e.detail)});const run=async()=>{s.textContent='Verifying…';try{await complete(await w.verify({timeout:180000}))}catch(e){fail()}};const t=setTimeout(fail,8000);customElements.whenDefined('sokol-captcha').then(()=>{clearTimeout(t);if(r.dataset.auto==='true')run();else s.textContent='Use the verification control when you are ready.'}).catch(fail);fetch('https://sokol.my-k.cloud/api/tools/whoami',{mode:'cors',cache:'no-store',credentials:'include'}).catch(blocked)})()</script>
</body></html>`
