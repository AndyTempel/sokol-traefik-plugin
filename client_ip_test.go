...
func TestForwardedClientIPWalksRightToLeft(t *testing.T) {
	...
	ip, err := extractClientIP(request, ClientIPConfig{Strategy: "forwarded"}, mustNetworks(t, "10.0.0.0/8"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestMalformedForwardedChainFallsBackToDirectPeer(t *testing.T) {
	...
	ip, err := extractClientIP(request, ClientIPConfig{Strategy: "forwarded"}, mustNetworks(t, "2001:db8::/32"))
	if err == nil {
...
func TestCloudflareAndBunnyHeadersRequireTrustedPeer(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			ip, err := extractClientIP(request, test.strategy, mustNetworks(t, "10.0.0.0/8"))
			if err != nil {
				t.Fatal(err)
			}
